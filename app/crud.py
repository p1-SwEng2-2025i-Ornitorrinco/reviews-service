...
async def get_reviews_with_user_info(service_id: ObjectId) -> List[dict]:
    """
    Obtiene las reviews de un servicio e incluye información del reviewer
    desde el servicio de usuarios
    """
    reviews = await get_reviews_by_service(service_id)
    
    # Enriquecer cada review con información del usuario
    for review in reviews:
        user_info = await users_client.get_user_basic_info(review["reviewer_id"])
        if user_info:
            review["reviewer_name"] = user_info.get("nombre_completo", "Usuario desconocido")
            review["reviewer_photo"] = user_info.get("foto_url")
        else:
            review["reviewer_name"] = "Usuario desconocido"
            review["reviewer_photo"] = None
